...
interface Props {
  ...
  onLoginClick: () => void; // login modal ochish uchun
}
...
export const LoginProviderTablet = ({ className, open, onOpenChange, onLoginClick }: Props) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogTrigger>{/* External trigger — qoladi */}</DialogTrigger>
      <DialogContent
        className={cn(
      ...
      >
        ...
        <p className="text-center font-thin mt-2">
          ...
          <button
            type="button"
            onClick={() => {
              onOpenChange(false); // Register modalni yopamiz
              setTimeout(() => {
                onLoginClick(); // Login modalni ochamiz
