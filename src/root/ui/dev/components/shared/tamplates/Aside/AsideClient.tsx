...
interface Props {
  ...
  token: string | null;
  categories: Category[];
  isMasterRegistered: boolean; // Serverdan kelgan qiymat
}

const Aside = ({ className, token, categories, isMasterRegistered }: Props) => {
  ...
  // Dinamik path uchun shart
  const masterLinkPath = isMasterRegistered ? "/myprofile" : "/myprofile/info";
        ...
        <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
          ...
          {token ? (
            <>
              ...
              <ServerLink path={masterLinkPath}>
  <span className="
    group 
    h-[42px] 
    px-2 
    ...
    bg-white
    border border-white
    animate-neon-glow-brighter
    
    // Hover holati
    hover:animate-neon-hover-brighter
    hover:bg-maket-primary
    hover:text-white
  ">
    <p className="
      relative 
      ...
      group-hover:drop-shadow-[0_0_4px_#32ADE6]
    ">
      Master bo'lib kirish
              ...
              {/* Optional Dialog (commented out, can be re-enabled if needed) */}
              {/* <Dialog>
                <DialogTrigger asChild>
                  <Button className="group bg-white h-[42px] px-2 flex items-center gap-2 rounded-md text-maket-primary border border-white hover:bg-maket-primary hover:text-white whitespace-nowrap transition-colors">
                    Master bo'lish
                  </Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader className="text-center">
                    <DialogTitle className="text-2xl font-bold text-[#001D55] text-center">
                      Master bo'lib kirish
                    </DialogTitle>
                  ...
                  <div className="flex-1 overflow-y-auto px-2 py-2">
                    <div className="flex flex-col gap-2">
                      <label className="text-sm font-medium w-28 h-28 text-gray-700 border-2 border-gray-300 rounded-[10px] flex flex-col items-center justify-center cursor-pointer hover:bg-gray-50 self-center">
                        <input
                          type="file"
                          accept="image/*"
                          multiple
