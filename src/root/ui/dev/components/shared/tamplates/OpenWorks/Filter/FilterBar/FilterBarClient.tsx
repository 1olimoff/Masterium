      ...
      <div className="basis-full md:basis-[27%] flex flex-col gap-1">
        ...
        <Popover open={catalogOpen} onOpenChange={setCatalogOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className="w-full text-[#a5abbd] hover:text-[#a5abbd] py-6 text-left text-md border-2 border-[#CFD9FE] rounded-xl flex justify-between items-center"
            >
          ...
          <PopoverContent className="p-2 w-full md:w-48">
            <Command>
              <CommandInput placeholder={t("OpenWorks.filter.catalog.search")} className="placeholder:text-[#677294]" />
              <CommandList>
          ...
          <Popover open={dateFromOpen} onOpenChange={setDateFromOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                className="w-full text-[#a5abbd] hover:text-[#a5abbd] py-6 text-left text-md border-2 border-[#CFD9FE] rounded-xl flex justify-between items-center"
              >
                {dateFrom ? dateFrom.toLocaleDateString() : t("OpenWorks.filter.price.from")}
                <Image src={"/svg/open-works/calendar.svg"} alt="Calendar icon" width={20} height={20} />
              </Button>
            </PopoverTrigger>
          ...
          <Popover open={dateToOpen} onOpenChange={setDateToOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                className="w-full text-[#a5abbd] hover:text-[#a5abbd] py-6 text-left text-md border-2 border-[#CFD9FE] rounded-xl flex justify-between items-center"
              >
                {dateTo ? dateTo.toLocaleDateString() : t("OpenWorks.filter.price.to")}
                <Image src={"/svg/open-works/calendar.svg"} alt="Calendar icon" width={25} height={25} />
              </Button>
            </PopoverTrigger>
            <PopoverContent className="p-2 w-auto" align="start">
              <Calendar
        ...
        <Popover open={locationOpen} onOpenChange={setLocationOpen}>
          <PopoverTrigger asChild>
            <Button
              ...
              className="w-full text-[#a5abbd] hover:text-[#a5abbd] py-6 text-left text-md border-2 border-[#CFD9FE] rounded-xl flex justify-between items-center"
            >
              {selectedLocation || t("OpenWorks.filter.location.placeholder")}
              <Image src={"/svg/open-works/arrow-down.svg"} alt="Arrow down icon" width={20} height={20} />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="p-2 w-full md:w-48">
            <Command>
              <CommandInput placeholder={t("OpenWorks.filter.location.placeholder")} />
              <CommandList>
                {locations.map((loc) => (
                  <CommandItem
                    key={loc}
