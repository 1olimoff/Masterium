...
export const OpenWorks = ({ className }: Props) => {
    ...
    return (
      <section className={cn(className, 'w-full flex flex-col gap-4 md:gap-6')}>
        <div className="w-full flex justify-between gap-2 md:gap-4">
          <h3 className="text-3xl font-semibold md:text-4xl">{t('OpenWorks.title')}</h3>
  ...
  <div
    className={cn(
      ...
      'md:gap-6 md:flex-row',     // ✅ Tablet: spacing + flex row
      'lg:grid lg:grid-cols-4 lg:gap-6' // ✅ Laptop: 4 column grid
    )}
  >
    {data.map((item, i) => (
      <div
        className={cn(
          'flex-shrink-0 w-[270px]', // ✅ Mobile: card eni
          'md:w-[300px]',            // ✅ Tablet: kengroq
