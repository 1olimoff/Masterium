...
const t = useTranslations("OfferWork.Advertise.breadcrumb")
  return (
    <Link href={"/tashkent"} className="flex items-center w-full bg-maket-primary sticky py-2 top-0 z-50 h-[35px] px-4 sm:hidden">
        <button className='flex items-center gap-2'> 
        <Image src="/svg/aside/MobileTab.svg" alt="Back" width={24} height={24} />
        <p className='text-white'>{t("advertise")}</p>
        </button>
    </Link>
  );
};
