...
interface Props {
  ...
  categories: { id: number; name: string }[]; // <-- TO‘G‘RILANDI
}
        ...
        <div className="py-4 bg-white m-3 rounded-[20px] px-2">
          <div className="flex gap-3">
            ...
            <div className="flex flex-col justify-between h-full my-2">
              ...
              <p className="text-maket-gray text-[14px]">{response.categories.map((cat) => cat.name).join(", ")}</p>
          ...
          <div>
            <div className="relative">
              <h2 className="text-[14px] mt-4 font-[#001D55]">{t("OfferWork.Offers.PersonProfile.WorkTitle.title")}</h2>
              <InputCount />
            </div>
            <div>
              <h2 className="text-[14px] font-[#001D55]">{t("OfferWork.Offers.PersonProfile.Category.title")}</h2>
